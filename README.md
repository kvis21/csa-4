...
- **`SET-ISR`** `( xt -- )` : Регистрирует токен исполнения `xt` как обработчик прерывания для вектора.
...
**Поток управления и система прерываний:** 
 ...
 -  При срабатывании прерывания процессор аппаратно сохраняет текущий PC и SR, а также R0-R5 на стек возвратов и переходит к обработчику; возврат из прерывания и восстановление контекста происходит по специальной инструкции IRET.
...
| EI | 0x42 | 1 | SR.IE <- 1 |
| DI | 0x43 | 1 | SR.IE <- 0 |
| IRET | 0x44 | 4 | SR <- mem[--RP] >, PC <- mem[--RP], IE <- 0  |
| HLT | 0x45| 1 | stop |
...
**защёлки регистров**: 
  - `latch_pc` — счётчик команд.
  - `latch_ir` — регистр текущей инструкции.
  - `latch_ar` — регистр адреса для работы с памятью данных.
  - `data_out` — внутренняя защёлка шины данных для промежуточного хранения при чтении из памяти.

**Регистр статуса (SR)**: хранит аппаратные флаги `N`, `Z`, `IE`, `INT`. Имеет аппаратную упаковку/распаковку в единое машинное слово для сохранения на стек при прерываниях (`out_sr` / `in_sr`).
...
**Доступ к памяти**:
  - Чтение инструкций: `IR <- IMEM[PC]`.
  - Работа с `DMEM`: адресация всегда происходит через защёлку `latch_ar`.
  - Чтение (`LD`, `POP`): `data_out <- DMEM[AR]`, затем значение переносится в целевой регистр `sel_rd`.
  - Запись (`ST`, `PUSH`): `DMEM[AR] <- sel_rs` или сохранение системных регистров (`PC`, `SR`).
...
**Декодер**: комбинационная логика на фазе `AF` извлекает номер целевого регистра `rd`, первого операнда `rs1`, флаг режима адресации `am` и константу/смещение.
...
**Цикл инструкции** динамический, занимает от 3 до 6 тактов:
  ...
  3. **EX (1–2 такта)**: Фаза исполнения.
    - Арифметические операции, ветвления и работа с портами ввода-вывода завершаются за 1 такт.
    ...
    - Возврат из прерывания (`IRET`) занимает 4 такта (`EX1-EX4`), поочередно восстанавливая `SR` и `PC` со стека `R7`.
...
Golden tests организованы в формате `tests/golden/<case>.yml`. Каждый YAML-файл содержит исходный forth код, входной поток, лимит тактов и ожидаемые результаты. Для каждого кейса проверяются:
1. вывод программы в 3 форматах: dec, hex, sym 
2. бинарная трансляция и debug dump
3. полный hex-образ бинарника, включая память команд и память данных
4. полный trace для всех кейсов

| Тест | Golden | Описание |
| :--- | :--- | :--- |
| **cat** | [cat.yml](tests/golden/cat.yml) | Потоковый ввод через IN и вывод через OUT. |
| **counter** | [counter.yml](tests/golden/counter.yml) | Тестирование механизма работы прерываний и EXECUTE. |
| **double** | [double.yml](tests/golden/double.yml) | арифметика 64-битных чисел на 32-битных словах. |
| **hello** | [hello.yml](tests/golden/hello.yml) | вывод строки в формате Pascal-string. |
| **hello_user_name** | [hello_user_name.yml](tests/golden/hello_user_name.yml) | Диалоговый ввод-вывод. |
| **sort** | [sort.yml](tests/golden/sort.yml) | пузырьковая сортировка списка чисел из ввода |
| **sum_square_dif** | [sum_square_dif.yml](tests/golden/sum_square_dif.yml) |  Euler Problem 6 - Математический расчет разности между квадратом суммы и суммой квадратов первых 100 натуральных чисел. |
...
: PRINT 
  ...
  REPEAT DROP DROP ;
: MAIN P"Hello World!" PRINT ;
MAIN
```

Компиляция и запуск:
```shell 
uv run python -m src.main translate examples/hello.s imem.bin dmem.bin debug.hex
uv run python -m src.main run  imem.bin dmem.bin
```

```
uv run python -m src.main translate examples/hello.s imem.bin dmem.bin debug.hex   
Чтение исходного кода из examples/hello.s
Генерация бинарника инструкций: imem.bin
Генерация бинарника данных: dmem.bin
Генерация hex-дампа (debug): debug.hex
    Инструкций (IMEM): 78 слов
    Данных (DMEM):     15 слов
=== Instruction Memory (IMEM) ===
0000 - 0x40000002 - JMP   2                        ; JMP to start
0001 - 0x40000002 - JMP   2                        ; JMP to INT 0
0002 - 0x60000048 - CALL  72                       ; Call entry point
0003 - 0x8A000000 - HLT                            ; Stop execution
0004 - 0x65400000 - PUSH  R5                       ; DUP: save NOS to mem
0005 - 0x23640000 - ADD   R5, R4, #0               ; DUP: NOS = TOS
...
=== Data Memory (DMEM) ===
0000 - 0x00000000 - 0 ('.')
0001 - 0x00000000 - 0 ('.')
0002 - 0x0000000C - 12 ('.')
0003 - 0x00000048 - 72 ('H')
0004 - 0x00000065 - 101 ('e')
0005 - 0x0000006C - 108 ('l')
0006 - 0x0000006C - 108 ('l')
0007 - 0x0000006F - 111 ('o')
0008 - 0x00000020 - 32 (' ')
0009 - 0x00000057 - 87 ('W')
0010 - 0x0000006F - 111 ('o')
0011 - 0x00000072 - 114 ('r')
0012 - 0x0000006C - 108 ('l')
0013 - 0x00000064 - 100 ('d')
0014 - 0x00000021 - 33 ('!')

uv run python -m src.main run  imem.bin dmem.bin -t trace.log
Hello World!
```

фрагмет трассировки:
```
Tick: 0001 |  JMP_IF  |  PC: 0001 | IR: 40000002 |  AR: 0000 | N:0 Z:0 IE:0 INT: 0 | R0:0000  R1:0000  R2:0000  R3:0000  R4:0000  R5:0000  R6:07FF  R7:0400
Tick: 0002 |  JMP_AF  |  PC: 0001 | IR: 40000002 |  AR: 0000 | N:0 Z:0 IE:0 INT: 0 | R0:0000  R1:0000  R2:0000  R3:0000  R4:0000  R5:0000  R6:07FF  R7:0400
Tick: 0003 |  JMP_1   |  PC: 0002 | IR: 40000002 |  AR: 0000 | N:0 Z:0 IE:0 INT: 0 | R0:0000  R1:0000  R2:0000  R3:0000  R4:0000  R5:0000  R6:07FF  R7:0400
Tick: 0004 | CALL_IF  |  PC: 0003 | IR: 60000048 |  AR: 0000 | N:0 Z:0 IE:0 INT: 0 | R0:0000  R1:0000  R2:0000  R3:0000  R4:0000  R5:0000  R6:07FF  R7:0400
Tick: 0005 | CALL_AF  |  PC: 0003 | IR: 60000048 |  AR: 0000 | N:0 Z:0 IE:0 INT: 0 | R0:0000  R1:0000  R2:0000  R3:0000  R4:0000  R5:0000  R6:07FF  R7:0400
Tick: 0006 |  CALL_1  |  PC: 0003 | IR: 60000048 |  AR: 03FF | N:0 Z:0 IE:0 INT: 0 | R0:0000  R1:0000  R2:0000  R3:0000  R4:0000  R5:0000  R6:07FF  R7:03FF
Tick: 0007 |  CALL_2  |  PC: 0048 | IR: 60000048 |  AR: 03FF | N:0 Z:0 IE:0 INT: 0 | R0:0000  R1:0000  R2:0000  R3:0000  R4:0000  R5:0000  R6:07FF  R7:03FF
Tick: 0008 | PUSH_IF  |  PC: 0049 | IR: 65400000 |  AR: 03FF | N:0 Z:0 IE:0 INT: 0 | R0:0000  R1:0000  R2:0000  R3:0000  R4:0000  R5:0000  R6:07FF  R7:03FF
Tick: 0009 | PUSH_AF  |  PC: 0049 | IR: 65400000 |  AR: 03FF | N:0 Z:0 IE:0 INT: 0 | R0:0000  R1:0000  R2:0000  R3:0000  R4:0000  R5:0000  R6:07FF  R7:03FF
Tick: 0010 |  PUSH_1  |  PC: 0049 | IR: 65400000 |  AR: 07FE | N:0 Z:0 IE:0 INT: 0 | R0:0000  R1:0000  R2:0000  R3:0000  R4:0000  R5:0000  R6:07FE  R7:03FF
Tick: 0011 |  PUSH_2  |  PC: 0049 | IR: 65400000 |  AR: 07FE | N:0 Z:0 IE:0 INT: 0 | R0:0000  R1:0000  R2:0000  R3:0000  R4:0000  R5:0000  R6:07FE  R7:03FF
```
Поля: `Tick` - счётчик тактов; стадия CU; PC - счетчик команд; IR - текущая инструкция; AR - регистр адреса; флаги: N, Z, IE, INT; регистры: R0-R7.