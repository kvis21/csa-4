...
- SP  — Выделенный аппаратный регистр. Указывает на вершину стека данных в DMEM.
- RP  — Выделенный аппаратный регистр. Указывает на вершину стека возвратов в DMEM.
...
| PUSH Rs | 0x32 | 2 | mem[SP] <- Rs; SP <- SP + 1 |
| POP Rd | 0x33 | 2 | SP <- SP - 1; Rd <- mem[SP] |
| IN Rd, #port | 0x40 | 1 | Rd <- port[port] |
| OUT Rs, #port | 0x41 | 1 | port[port] <- Rs |
| EI | 0x42 | 1 | SR.IE <- 1 |
| DI | 0x43 | 1 | SR.IE <- 0 |
| IRET | 0x44 | 4 | RP <- RP - 1; SR <- mem[RP]; RP <- RP - 1; PC <- mem[RP] |
